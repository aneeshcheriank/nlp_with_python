...
def test_search_wikipedia():
    ...
    with patch("wikipedia.search", return_value=mock_return_value) as mock_search:
        ...
        # Assertion
        mock_search.assert_called_once_with("Python") # Ensure it was called with the expected argument
...
def test_summarize_wikipedia():
    ...
    with patch('wikipedia.summary', return_value=mock_return_value) as mock_summary:
        ...
        mock_summary.assert_called_once_with('Facebook', sentences=2) # need to be called with the same arguments as the mocking fuction
